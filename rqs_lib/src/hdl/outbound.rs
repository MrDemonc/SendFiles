        ...
        match &self.payload {
            OutboundPayload::Files(files) => {
                for f in files {
                    ...
                    let mut ftype = mime_guess::from_path(path)
                        .first_or_octet_stream()
                        .to_string();

                    let extension = path.extension().and_then(|s| s.to_str()).unwrap_or_default().to_lowercase();
                    
                    let meta_type = if ftype.starts_with("image/") || matches!(extension.as_str(), "webp" | "svg" | "ico" | "tga" | "psd" | "ai") {
                        if extension == "webp" { ftype = "image/webp".to_string(); }
                        else if extension == "svg" { ftype = "image/svg+xml".to_string(); }
                        file_metadata::Type::Image
                    } else if ftype.starts_with("video/") || matches!(extension.as_str(), "mkv" | "webm" | "mov" | "avi" | "flv") {
                        file_metadata::Type::Video
                    } else if ftype.starts_with("audio/") || matches!(extension.as_str(), "flac" | "wav" | "ogg" | "m4a" | "aac") {
                        file_metadata::Type::Audio
                    } else if matches!(extension.as_str(), "apk" | "apkm" | "xapk" | "deb" | "rpm" | "msi" | "ipa" | "appx" | "dmg" | "pkg" | "exe" | "jar") {
                        match extension.as_str() {
                            "apk" | "apkm" | "xapk" => ftype = "application/vnd.android.package-archive".to_string(),
                            "deb" => ftype = "application/x-debian-package".to_string(),
                            "rpm" => ftype = "application/x-rpm".to_string(),
                            "msi" => ftype = "application/x-msi".to_string(),
                            "dmg" => ftype = "application/x-apple-diskimage".to_string(),
                            "jar" => ftype = "application/java-archive".to_string(),
                            "exe" => ftype = "application/x-msdownload".to_string(),
                            _ => {}
                        }
                        file_metadata::Type::App
                    } else {
                        // Universal exhaustive mapping for other types
                        match extension.as_str() {
                            // Documents & Books
                            "pdf" => ftype = "application/pdf".to_string(),
                            "epub" => ftype = "application/epub+zip".to_string(),
                            "mobi" => ftype = "application/x-mobipocket-ebook".to_string(),
                            "rtf" => ftype = "application/rtf".to_string(),
                            "txt" | "log" | "env" | "ini" | "conf" => ftype = "text/plain".to_string(),
                            
                            // Archives
                            "zip" => ftype = "application/zip".to_string(),
                            "rar" => ftype = "application/x-rar-compressed".to_string(),
                            "7z" => ftype = "application/x-7z-compressed".to_string(),
                            "tar" => ftype = "application/x-tar".to_string(),
                            "gz" => ftype = "application/gzip".to_string(),
                            
                            // Office Suites
                            "doc" | "docx" => ftype = "application/msword".to_string(),
                            "xls" | "xlsx" => ftype = "application/vnd.ms-excel".to_string(),
                            "ppt" | "pptx" => ftype = "application/vnd.ms-powerpoint".to_string(),
                            "odt" => ftype = "application/vnd.oasis.opendocument.text".to_string(),
                            "ods" => ftype = "application/vnd.oasis.opendocument.spreadsheet".to_string(),
                            "odp" => ftype = "application/vnd.oasis.opendocument.presentation".to_string(),
                            
                            // Web & Development
                            "html" | "htm" => ftype = "text/html".to_string(),
                            "xml" => ftype = "text/xml".to_string(),
                            "json" | "cjson" => ftype = "application/json".to_string(),
                            "js" | "ts" | "jsx" | "tsx" | "mjs" | "cjs" => ftype = "text/javascript".to_string(),
                            "css" | "scss" | "less" => ftype = "text/css".to_string(),
                            "php" => ftype = "application/x-httpd-php".to_string(), // PHP can be sensitive
                            "rs" => ftype = "text/x-rust".to_string(),
                            "py" => ftype = "text/plain".to_string(), // Generic for Android compatibility
                            "c" | "cpp" | "h" | "hpp" | "cc" | "hh" => ftype = "text/x-c".to_string(),
                            "java" | "kt" | "kts" => ftype = "text/x-java".to_string(),
                            "go" => ftype = "text/x-go".to_string(),
                            "sh" | "bash" | "zsh" | "fish" => ftype = "text/plain".to_string(), // Generic to avoid block
                            "sql" | "db" | "sqlite" | "sqlite3" => ftype = "text/x-sql".to_string(),
                            "yaml" | "yml" => ftype = "text/yaml".to_string(),
                            "toml" => ftype = "text/x-toml".to_string(),
                            "md" | "markdown" => ftype = "text/markdown".to_string(),
                            "graphql" | "gql" => ftype = "application/graphql".to_string(),
                            "dockerfile" => ftype = "text/x-dockerfile".to_string(),
                            
                            _ => ftype = mime_guess::from_path(path).first_or_octet_stream().to_string(),
                        }
